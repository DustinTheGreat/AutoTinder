...
headers = {
	...
	"X-Auth-Token": "ea9e48b1-9108-4865-9f66-cd8983e9e1ea",
...
def get_updates(last_activity_date):
    ...
    try:
        ...
        '''for x in r.json()['matches']:
            ...
            print("\n")
            print("\n")'''
        for x in r.json()['matches']['messages']:
            print(x)
...
def send_msg(msg, my_id, their_id):

    try:
        url = config.host +"/user/matches/" + my_id +their_id
        print(url)
        r = requests.post(url, headers=headers,
                          data=json.dumps({"message": msg}))
        #add try for response code for json
    except requests.exceptions.RequestException as e:
...
def all_matches():
    count = 20
    try:
        url = config.host + '/v2/matches?count={}'.format(count)
        r = requests.get(url, headers=headers)
        data = r.json()
        all_ids = []
        for x in range(count):
            id_ = data['data']['matches'][x]['person']['_id']
            name =data['data']['matches'][x]['person']['name']
            all_ids.append((id_, name))
        return(all_ids)
    except requests.exceptions.RequestException as e:
        print("Something went wrong. Could not get your match info:", e)


#get_person("5b46bf9c4f0e83280552b4f9")
...
def get_message_updates(time):

    try:
        url = config.host + "/updates"
        r = requests.post(url, headers=headers,
                          data=json.dumps({"last_activity_date": time}))
        #add try for response code for json
        #print(r.json())
        new_updates = (len(r.json()['matches']))
        response = []
        for x in range(new_updates -4):

            #print(r.json()['matches'][x]['messages'][-1])
            #reponse.append((r.json()['matches'][x]['messages'][-1]['from']))
            if (r.json()['matches'][x]['messages'][-1]['from']) != my_id:
                response.append([(r.json()['matches'][x]['messages'][-1])])

        reply_to_response(response)

           
    except requests.exceptions.RequestException as e:
        print("Something went wrong. Could not send your message:", e)

def reply_to_response(people):
    for x in people:
...
def message_all_matches():
    for x in all_matches():
        id_ = x[0]
        msg = "hey {}".format(x[1])
        try:
            send_msg(msg, me, id_)
            print("sent {} to {}".format(msg, id_))
        #this is so if the users ID comes before yours in the url, temp fix
        except:
             send_msg(msg, id_,  me)
             print("sent {} to {}".format(msg, id_))
...
def messsage_new_matches():
    pass


#get_new_matches('2019-10-18T23:31:20.451Z')

#get_message_updates("2019-10-19T01:16:47.822Z")
get_message_updates('2019-10-19T01:16:47.822Z')
#get_person('5d5a29f1b1a39f160005b31f')
#get_recommendations()
#send_msg('hey minyy' me, "5d807ffa50e984e8191e719d")
#message_all_matches()
...
577489c77c74ea693ee9fe215c69de0e5c63011100d0a406/messages?count=100&locale=en&
page_token=MjAxOS0xMC0xOVQyMjo0MDo0OC45MTda
'''